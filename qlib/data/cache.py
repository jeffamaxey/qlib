...
class BaseProviderCache:
    ...
    def check_cache_exists(cache_path: Union[str, Path], suffix_list: Iterable = (".index", ".meta")) -> bool:
        ...
        return all(
            p.exists()
            for p in [cache_path]
            + [cache_path.with_suffix(_s) for _s in suffix_list]
        )
    ...
    def _expression(self, instrument, field, start_time=None, end_time=None, freq="day"):
        ...
        if self.check_cache_exists(cache_path, suffix_list=[".meta"]):
            ...
            try:
                ...
                return read_bin(cache_path, start_index, end_index)
            except Exception:
                series = None
                self.logger.error(
                    f"reading {cache_path} file error : {traceback.format_exc()}"
                )
            return series
        else:
            # normalize field
            ...
            if isinstance(eval(parse_field(field)), Feature):
